...
class GenericUri
{
  public:
    ...
    GenericUri(Allocator *allocator = 0)
        : uri_(), base_(), scheme_(), auth_(), path_(), query_(), frag_(),
          allocator_(allocator), ownAllocator_()
    {
    }
    ...
    {
        const Ch *u =
            uri.template Get<const Ch *>(); // TypeHelper from document.h
    ...
    {
        uri.template Set<const Ch *>(this->GetString(),
                                     allocator); // TypeHelper from document.h
    }
    ...
    //! Resolve this URI against another (base) URI in accordance with URI
    //! resolution rules.
    ...
    GenericUri Resolve(const GenericUri &baseuri, Allocator *allocator = 0)
    {
        ...
        resuri.Allocate(GetStringLength() + baseuri.GetStringLength() +
                        1); // + 1 for joining slash

        if (!(GetSchemeStringLength() == 0)) {
            // Use all of this URI
            resuri.auth_ =
                CopyPart(resuri.scheme_, scheme_, GetSchemeStringLength());
            resuri.path_ = CopyPart(resuri.auth_, auth_, GetAuthStringLength());
            resuri.query_ =
                CopyPart(resuri.path_, path_, GetPathStringLength());
            resuri.frag_ =
                CopyPart(resuri.query_, query_, GetQueryStringLength());
            resuri.RemoveDotSegments();
        } else {
            // Use the base scheme
            resuri.auth_ = CopyPart(resuri.scheme_, baseuri.scheme_,
                                    baseuri.GetSchemeStringLength());
            if (!(GetAuthStringLength() == 0)) {
                // Use this auth, path, query
                resuri.path_ =
                    CopyPart(resuri.auth_, auth_, GetAuthStringLength());
                resuri.query_ =
                    CopyPart(resuri.path_, path_, GetPathStringLength());
                resuri.frag_ =
                    CopyPart(resuri.query_, query_, GetQueryStringLength());
                ...
                // Use the base auth
                resuri.path_ = CopyPart(resuri.auth_, baseuri.auth_,
                                        baseuri.GetAuthStringLength());
                if (GetPathStringLength() == 0) {
                    // Use the base path
                    resuri.query_ = CopyPart(resuri.path_, baseuri.path_,
                                             baseuri.GetPathStringLength());
                    if (GetQueryStringLength() == 0) {
                        // Use the base query
                        resuri.frag_ = CopyPart(resuri.query_, baseuri.query_,
                                                baseuri.GetQueryStringLength());
                    } else {
                        // Use this query
                        resuri.frag_ = CopyPart(resuri.query_, query_,
                                                GetQueryStringLength());
                    }
                } else {
                    if (path_[0] == '/') {
                        // Absolute path - use all of this path
                        resuri.query_ = CopyPart(resuri.path_, path_,
                                                 GetPathStringLength());
                        resuri.RemoveDotSegments();
                    } else {
                        // Relative path - append this path to base path after
                        // base path's last slash
                        size_t pos = 0;
                        ...
                        std::memcpy(&resuri.path_[pos], baseuri.path_,
                                    lastslashpos * sizeof(Ch));
                        pos += lastslashpos;
                        resuri.query_ = CopyPart(&resuri.path_[pos], path_,
                                                 GetPathStringLength());
                        resuri.RemoveDotSegments();
                    }
                    // Use this query
  ...
  private:
    ...
    std::size_t Allocate(std::size_t len)
    {
        ...
        // Allocate one block containing each part of the URI (5) plus base plus
        // full URI, all null terminated. Order: scheme, auth, path, query,
        // frag, base, uri
    ...
    // parts Supports URIs that match regex
    // ^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))? as per
    ...
    void RemoveDotSegments()
    {
        ...
        while (pathpos < pathlen) {
            ...
            if (slashpos == 2 && path_[pathpos] == '.' &&
                path_[pathpos + 1] == '.') {
                // Backup a .. segment in the new path_
                // We expect to find a previously added slash at the end or
                // nothing
                ...
                std::memmove(&path_[newpos], &path_[pathpos],
                             slashpos * sizeof(Ch));
                newpos += slashpos;
