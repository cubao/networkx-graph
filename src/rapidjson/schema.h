...
template <typename SchemaDocumentType> class Schema
{
  public:
    ...
    Schema(SchemaDocumentType *schemaDocument, const PointerType &p,
           ...
           AllocatorType *allocator, const UriType &id = UriType())
        : allocator_(allocator), uri_(schemaDocument->GetURI(), *allocator),
          id_(id), pointer_(p, allocator),
          typeless_(schemaDocument->GetTypeless()), enum_(), enumCount_(),
          ...
          exclusiveMaximum_(false), defaultValueLength_(0)
    {
        ...
        // PR #1393
        // Early add this Schema and its $ref(s) in schemaDocument's map to
        // avoid infinite recursion (with recursive schemas), since
        // schemaDocument->getSchema() is always checked before creating a new
        // one. Don't cache typeless_, though.
        if (this != typeless_) {
            typedef typename SchemaDocumentType::SchemaEntry SchemaEntry;
            SchemaEntry *entry =
                schemaDocument->schemaMap_.template Push<SchemaEntry>();
...
class IGenericRemoteSchemaDocumentProvider
{
  public:
    ...
    GetRemoteDocument(GenericUri<ValueType, AllocatorType> uri)
    {
        return GetRemoteDocument(uri.GetBaseString(),
...
class GenericSchemaDocument
{
  public:
    ...
    /*!
       ...
       allocator instance for allocating memory. Can be null. \param pointer An
       optional JSON pointer to the start of the schema document
    */
          ...
          schemaRef_(allocator, kInitialSchemaRefSize)
    {
        ...
        // sub-schemas, And call HandleRefSchema() if there are $ref. PR #1393
        // use input pointer if supplied
        root_ = typeless_;
  ...
  private:
                                ...
                                const ValueType &document, const UriType &id)
    {
        ...
        if (v.IsObject()) {
            if (const SchemaType *sc = GetSchema(pointer)) {
                ...
                // The new schema constructor adds itself and its $ref(s) to
                // schemaMap_
                         ...
                         const UriType &id)
    {
        typename ValueType::ConstMemberIterator itr =
            v.FindMember(SchemaType::GetRefString());
        if (itr == v.MemberEnd())
            return false;
        ...
        if (itr->value.IsString()) {
            ...
            if (len > 0) {
                ...
                UriType ref = UriType(itr->value, allocator_)
                                  .Resolve(scopeId, allocator_);
                // See if the resolved $ref minus the fragment matches a
                // resolved id in this document Search from the root. Returns
                // the subschema in the document and its absolute JSON pointer.
                PointerType basePointer = PointerType();
                const ValueType *base =
                    FindId(document, ref, basePointer, docId_, false);
                if (!base) {
                    // Remote reference - call the remote document provider
                    if (remoteProvider_) {
                        if (const GenericSchemaDocument *remoteDocument =
                                remoteProvider_->GetRemoteDocument(ref)) {
                            const Ch *s = ref.GetFragString();
                            len = ref.GetFragStringLength();
                            if (len <= 1 || s[1] == '/') {
                                // JSON pointer fragment, absolute in the remote
                                // schema
                                const PointerType pointer(s, len, allocator_);
                                if (pointer.IsValid()) {
                                    // Get the subschema
                                    if (const SchemaType *sc =
                                            remoteDocument->GetSchema(
                                                pointer)) {
                                        if (schema)
                                            *schema = sc;
                                        AddSchemaRefs(
                    ...
                    if (len <= 1 || s[1] == '/') {
                        ...
                        if (relPointer.IsValid()) {
                            ...
                            if (const ValueType *pv = relPointer.Get(*base)) {
                                // Now get the absolute JSON pointer by adding
                                // relative to base
                                PointerType pointer(basePointer);
                                for (SizeType i = 0;
                                     i < relPointer.GetTokenCount(); i++)
                                    pointer = pointer.Append(
                                        relPointer.GetTokens()[i], allocator_);
                                // GenericStringBuffer<EncodingType> sb;
                                // pointer.StringifyUriFragment(sb);
                                if (pointer.IsValid() &&
                                    !IsCyclicRef(pointer)) {
                                    // Call CreateSchema recursively, but first
                                    // compute the in-scope id for the $ref
                                    // target as we have jumped there
                                    // TODO: cache pointer <-> id mapping
                                    size_t unresolvedTokenIndex;
                                    scopeId = pointer.GetUri(
                                        document, docId_, &unresolvedTokenIndex,
                                        allocator_);
                                    CreateSchema(schema, pointer, *pv, document,
                                                 scopeId);
                                    return true;
                                }
                            }
                        ...
                        // Search from the base we just established. Returns the
                        // subschema in the document and its absolute JSON
                        // pointer.
                        PointerType pointer = PointerType();
                        if (const ValueType *pv = FindId(
                                *base, ref, pointer,
                                UriType(ref.GetBaseString(),
                                        ref.GetBaseStringLength(), allocator_),
                                true, basePointer)) {
                            if (!IsCyclicRef(pointer)) {
                                // GenericStringBuffer<EncodingType> sb;
                                // pointer.StringifyUriFragment(sb);
                                //  Call CreateSchema recursively, but first
                                //  compute the in-scope id for the $ref target
                                //  as we have jumped there
                                //  TODO: cache pointer <-> id mapping
                                size_t unresolvedTokenIndex;
    ...
    //! Find the first subschema with a resolved 'id' that matches the specified
    //! URI.
    // If full specified use all URI else ignore fragment.
    // If found, return a pointer to the subschema and its JSON pointer.
    // TODO cache pointer <-> id mapping
    ValueType *FindId(const ValueType &doc, const UriType &finduri,
                      PointerType &resptr, const UriType &baseuri, bool full,
                      const PointerType &here = PointerType()) const
    {
        ...
        if (doc.GetType() == kObjectType) {
            ...
            typename ValueType::ConstMemberIterator m =
                doc.FindMember(SchemaType::GetIdString());
            ...
            for (m = doc.MemberBegin(); m != doc.MemberEnd(); ++m) {
                if (m->value.GetType() == kObjectType ||
                    m->value.GetType() == kArrayType) {
                    resval = FindId(m->value, finduri, resptr, localuri, full,
                                    here.Append(m->name.GetString(),
            ...
            // Continue looking
            for (typename ValueType::ConstValueIterator v = doc.Begin();
                 v != doc.End(); ++v) {
                if (v->GetType() == kObjectType || v->GetType() == kArrayType) {
                    resval = FindId(*v, finduri, resptr, localuri, full,
                                    here.Append(i, allocator_));
                }
                if (resval)
                    break;
                i++;
    ...
    {
        for (const SchemaRefPtr *ref =
                 schemaRef_.template Bottom<SchemaRefPtr>();
    ...
    internal::Stack<Allocator>
        schemaRef_; // Stores Pointer(s) from $ref(s) until resolved
