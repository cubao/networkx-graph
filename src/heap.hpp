...
//
// The most efficient is the Fibonacci heap, which allows the first operation to
// run in   $O(\log n)$ , and the second operation in   $O(1)$ . Therefore we
// will get the complexity   $O(n \log n + m)$  for Dijkstra's algorithm, which
// is also the theoretical minimum for the shortest path search problem.
// Therefore this algorithm works optimal, and Fibonacci heaps are the optimal
// data structure. There doesn't exist any data structure, that can perform both
// operations in   $O(1)$ , because this would also allow to sort a list of
// random numbers in linear time, which is impossible. Interestingly there
// exists an algorithm by Thorup that finds the shortest path in   $O(m)$  time,
// however only works for integer weights, and uses a completely different idea.
// So this doesn't lead to any contradictions. Fibonacci heaps provide the
...
template <class T, class Comp = std::less<T>> class FibHeap
{
  public:
    ...
    class FibNode
    {
      public:
        FibNode(T k, void *pl)
            : key(std::move(k)), mark(false), p(nullptr), left(nullptr),
              right(nullptr), child(nullptr), degree(-1), payload(pl)
        {
        }
    ...
    FibHeap(Comp comp) : n(0), min(nullptr), comp(comp) {}
    ...
    static FibHeap *union_fibheap(FibHeap *H1, FibHeap *H2)
    {
        ...
        // 4
        if (H1->min == nullptr ||
            (H2->min != nullptr && H1->comp(H2->min->key, H1->min->key))) {
            // 5
            H->min = H2->min;
        }
        // 6
    ...
    void consolidate()
    {
        ...
        // 1
        A = new FibNode
            *[max_degree + 2]; // plus two both for indexing to max degree and
                               // so A[max_degree+1] == NIL
        // 2, 3
