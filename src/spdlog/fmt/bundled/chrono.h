...
template <typename OutputIt, typename Char> class tm_writer
{
  ...
  public:
    ...
    {
        if (is_classic_ || ns == numeric_system::standard)
            return write2((tm_yday() + days_per_week - tm_wday()) /
                          days_per_week);
        format_localized('U', 'O');
    }
    ...
    {
        char buf[8];
        write_digit2_separated(buf, to_unsigned(tm_hour()),
                               to_unsigned(tm_min()), to_unsigned(tm_sec()),
                               ':');
        out_ = copy_str<Char>(std::begin(buf), std::end(buf), out_);
    }
...
struct chrono_formatter
{
    ...
    {
        constexpr auto num_fractional_digits = count_fractional_digits(
        ...
        if (std::ratio_less<typename subsecond_precision::period,
            ...
            *out++ = '.';
            ...
            auto fractional =
                detail::abs(d) - std::chrono::duration_cast<sec>(d);
            const auto subseconds =
                std::chrono::treat_as_floating_point<
                    ...
                    : std::chrono::duration_cast<subsecond_precision>(
                          fractional)
                          .count();
            uint32_or_64_or_128_t<long long> n = to_unsigned(
                to_nonnegative_int(subseconds, max_value<long long>()));
            int num_digits = detail::count_digits(n);
  ...
  public:
        ...
        -> decltype(ctx.out())
    {
        ...
        detail::handle_dynamic_spec<detail::precision_checker>(
            precision_copy, precision_ref, ctx);
        if (begin == end || *begin == '}') {
            out = detail::format_duration_value<Char>(out, d.count(),
                                                      precision_copy);
...
template <typename Char> struct formatter<std::tm, Char>
{
  ...
  public:
        ...
        -> decltype(ctx.out())
    {
        ...
        auto w =
            detail::tm_writer<decltype(ctx.out()), Char>(loc, ctx.out(), tm);
