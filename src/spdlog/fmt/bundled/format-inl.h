...
{
    static FMT_CONSTEXPR_DECL const int value = static_cast<int>(
...
struct fp
{
    ...
    // Assigns d to this and return true iff predecessor is closer than
    // successor.
...
class bigint
{
  private:
    ...
    FMT_CONSTEXPR20 void multiply(uint64_t value)
    {
        ...
        for (size_t i = 0, n = bigits_.size(); i < n; ++i) {
            ...
            carry = bigits_[i] * upper + (result >> bigit_bits) +
                    (carry >> bigit_bits);
            bigits_[i] = static_cast<bigit>(result);
        }
        while (carry != 0) {
  ...
  public:
    ...
    FMT_CONSTEXPR20 void square()
    {
        ...
        using accumulator_t =
            conditional_t<FMT_USE_INT128, uint128_t, accumulator>;
...
template <> struct cache_accessor<float>
{
    ...
    {
        return (static_cast<carrier_uint>(
                    cache >> (64 - float_info<float>::significand_bits - 2 -
                              beta_minus_1)) +
                1) /
               2;
    }
...
template <> struct cache_accessor<double>
{
                                   ...
                                   int beta_minus_1) FMT_NOEXCEPT
    {
        ...
        return ((umul192_middle64(two_f, cache) >> (64 - beta_minus_1)) & 1) !=
               0;
    }
    ...
    compute_round_up_for_shorter_interval_case(const cache_entry_type &cache,
                                               int beta_minus_1) FMT_NOEXCEPT
    {
        return ((cache.high() >> (64 - float_info<double>::significand_bits -
                                  2 - beta_minus_1)) +
                1) /
               2;
    }
...
bool is_left_endpoint_integer_shorter_interval(int exponent) FMT_NOEXCEPT
{
    return exponent >=
           ...
           exponent <=
               float_info<
                   T>::case_shorter_interval_left_endpoint_upper_threshold;
}
template <class T>
...
template <typename T> decimal_fp<T> to_decimal(T x) FMT_NOEXCEPT
{
    ...
    if (exponent != 0) { // Check if normal.
        exponent +=
            float_info<T>::exponent_bias - float_info<T>::significand_bits;
        ...
        if (significand == 0)
            return {0, 0};
        exponent =
            float_info<T>::min_exponent - float_info<T>::significand_bits;
    }
    ...
    const uint32_t deltai =
        cache_accessor<T>::compute_delta(cache, beta_minus_1);
    ...
    if (r > deltai) {
        ...
        if ((!include_left_endpoint ||
            ...
            !cache_accessor<T>::compute_mul_parity(two_fl, cache,
                                                   beta_minus_1)) {
            goto small_divisor_case_label;
        }
    }
...
small_divisor_case_label:
    ...
    if ((dist & mask) == 0) {
        ...
        if (check_divisibility_and_divide_by_pow5<float_info<T>::kappa>(dist)) {
            ...
            // Since there are only 2 possibilities, we only need to care about
            // the parity. Also, zi and r should have the same parity since the
            // divisor is an even number
            if (cache_accessor<T>::compute_mul_parity(
                    two_fc, cache, beta_minus_1) != approx_y_parity) {
                --ret_value.significand;
            } else {
